<?php
// Works 一覧 — 全作品をテーブル表示し、編集・新規追加へのリンクを提供する
...
$db    = get_db();
$rows  = $db->query("SELECT id, slug, title, date, tags, pinned, updated_at FROM works ORDER BY pinned DESC, date DESC")->fetchAll();
  ...
  <table class="data-table data-table--clickable">
    ...
    <tbody>
      ...
      <tr data-href="<?= htmlspecialchars($edit_url, ENT_QUOTES) ?>" class="clickable-row">
        ...
        <td>
          <?php if ($row['pinned']): ?>
          <span class="badge badge--accent">固定</span>
          <?php endif; ?>
        </td>
